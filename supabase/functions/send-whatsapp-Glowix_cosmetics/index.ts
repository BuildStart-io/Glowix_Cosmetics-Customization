    ...
    if (url) {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          console.warn(`[send-whatsapp] WAHA media send failed (${res.status}): ${errText.substring(0, 150)}. Falling back to sendText.`);
          const fallbackText = message ? `${message}\n\n${url}` : url;
          res = await wahaFetch("/api/sendText", { session: sessionName, chatId, text: fallbackText });
        }
      } catch (mediaErr: any) {
        console.warn(`[send-whatsapp] Media dispatch threw error (${mediaErr.message}). Falling back to sendText.`);
        const fallbackText = message ? `${message}\n\n${url}` : url;
        res = await wahaFetch("/api/sendText", { session: sessionName, chatId, text: fallbackText });
      }
    } else {
